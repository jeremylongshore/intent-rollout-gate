 ...
 * sigstore bundles into the final `report-manifest.json` the intent-eval-
 * dashboard fetches for repo key `iar`.
 *
 * Lives in `ci/` (CI-only surface, mirroring audit-harness). Runs AFTER
 * `ci/emit-evidence.ts` and AFTER CI has signed each `bundle-<i>.json` with
 * `cosign sign-blob --bundle bundle-<i>.sigstore.json`. Fail-closed: a missing
 * bundle/sigstore file or a structural mismatch aborts rather than publishing
...
export function assemble(dir: string): ReportManifest {
  ...
  const rows: ManifestRow[] = skeleton.rows.map((row) => {
    ...
    const sigPath = join(
      dir,
      `${basename(row.bundleFile, ".json")}.sigstore.json`,
    );
...
const invokedDirectly =
  process.argv[1]?.endsWith("assemble-manifest.ts") === true;
if (invokedDirectly) {
  try {
    ...
    console.log(
      `✓ assemble-manifest: ${manifest.rows.length} signed row(s) -> ${out}`,
    );
    process.exit(0);
  } catch (err: unknown) {
    console.error(
