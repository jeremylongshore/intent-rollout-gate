...
| Action mechanism (today) | `runs.using: composite` with a single `bash` step | `action.yml:73-87` | Bootstrap no-op; declare full input/output surface without committing to a runtime | Composite actions can declare the manifest contract without any compiled code; the cost is that they cannot do real work (no access to `@actions/core`-equivalent APIs in a polished form). Acceptable for a stub. |
