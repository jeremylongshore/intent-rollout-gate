...
The gate makes no novel attestations of code quality. It is strictly a **decision tier** above the bundle: the bundle is the input, the policy is the threshold, the decision is the output. The decision itself is an attestation (signed in-toto row at `https://evals.intentsolutions.io/rollout-decision/v1`) so it is independently verifiable downstream.
...
The gate **does not require complete coverage** (per Evidence Bundle SPEC R2 / system brief § 8 ¶3). A partial bundle is valid input; the policy decides whether the partial coverage is sufficient.
...
Rows that fail any check are dropped from the eligible set with a clear note in the decision summary. The bundle as a whole does not fail merely because some rows fail verification — partial verification is a recoverable condition, the policy decides whether the verified subset is sufficient.
...
These constraints are inherited from ISEDC Decision Record 004 § 6 ([`intent-eval-lab/000-docs/004-AT-DECR-isedc-council-record-2026-05-10.md`](https://github.com/jeremylongshore/intent-eval-lab/blob/main/000-docs/004-AT-DECR-isedc-council-record-2026-05-10.md)). They are **non-negotiable** for any implementation PR.
