...
- **Adversary inside the consumer repo** — AI agent or contributor attempting to lower thresholds in `tests/TESTING.md`, edit the bundle directory before the gate reads it, or pass a forged decision row downstream. **Mitigation at v0.3.0 (honest scope):** the action does **not** yet verify bundle-row cryptographic signatures — that is a **deferred** feature (DR-002 § 6.3; tracked). What the gate does today: it reads exactly one policy source, delegates the ship/no-ship decision wholly to `@intentsolutions/rollout-gate` `decide()`, and runs each consumed `gate-result/v1` predicate body through an **advisory** kernel Zod check (`countKernelInvalidPredicates`) that surfaces a `core.warning` on a structurally-malformed body but never changes the decision. Fail-closed wiring blocks on an unreadable / invalid-JSON bundle, on a missing-or-both policy source, and on an unsupported `predicate-uri`. **Residual risk until signature verification lands:** a consumer-repo adversary who can rewrite the bundle file before the gate reads it can influence the decision — so the bundle's provenance must be pinned **upstream** (the producing tool signs to Rekor) and the gate's input treated as trusted-by-construction in the CI graph. Decision-row signing (`signed-decision-row-path`) is likewise reserved/empty at v0.3.0.
- **Adversary upstream of the gate** — a tool that emits Evidence Bundle rows with a forged predicate URI (e.g., a row claiming `predicateType: https://evals.intentsolutions.io/gate-result/v1` but with a body that doesn't match the schema). **Mitigation at v0.3.0 (honest scope):** there is **no Stage-1 signature / subject-digest verification yet** (deferred, DR-002 § 6.3). If the action's `predicate-uri` **input parameter** is set to an unsupported value, the action blocks (a wiring-level guard — not a per-row check). Bundle **rows** whose `predicateType` is anything other than `gate-result/v1` are **not** rejected: they are skipped by the advisory kernel check and passed through to the decision engine. For rows that *do* claim `gate-result/v1`, the only structural defense is the **advisory** kernel-schema check: a body that fails `GateResultV1Schema` is counted and surfaced as a warning, but it is **not** rejected and does **not** block. Rejecting rows on invalid signatures, missing required predicate fields, or subject-digest mismatches is the deferred Stage-1 work tracked alongside the signature-verification feature above.
